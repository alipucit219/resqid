...
const VerticalNavItems = (props: Props) => {
  ...
  const [, setModifyverticalNavitems] = useState<any>([])
  const filterOptions = () => {
    const userPermissionKeys = Object.keys(user?.user?.permissions || {})
    const filteredOptions = verticalNavItems?.filter((item: any) => {
      return userPermissionKeys.includes(item?.title)
    })
    setModifyverticalNavitems(filteredOptions)
  }
  useEffect(() => {
    filterOptions()
  }, [user, verticalNavItems])

  const RenderMenuItems = verticalNavItems?.map((item: NavGroup | NavLink | NavSectionTitle, index: number) => {
    // console.log('🚀 ~ file: VerticalNavItems.tsx:53 ~ RenderMenuItems ~ item:', item)
